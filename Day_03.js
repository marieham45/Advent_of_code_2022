...
const splitInHalf = (arr) => {
    ...
    let arrayOfHalves = [];
        ...
        for (let i = 0; i < arr.length; i++) {
            const firstHalf = arr[i].slice(0, arr[i].length / 2).split("");
            const secondHalf = arr[i].slice(arr[i].length / 2).split("");
            
            const twoCompartmentsArray = [firstHalf, secondHalf]
            
            arrayOfHalves.push(twoCompartmentsArray);
    }
    return arrayOfHalves;
}

const findSameValues = (arr) => {
    let sameItemsString = "";
    let sameValue = "";
    for (let i = 0; i < arr.length; i++) {
        for (let m = 0; m < arr[i][0].length; m++) {
            if (arr[i][1].includes(arr[i][0][m])) {
                sameValue = arr[i][0][m];

            }

        }
    sameItemsString += sameValue;
    ...
    for (let j = 0; j < sameItemsString.length; j++) {

    switch (sameItemsString[j]) {
            case "a":
            sum += 1;
            break;
            case "b":
...
console.log(findSameValues(splitInHalf(testArray)))
