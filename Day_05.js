const testString = `move 3 from 5 to 7
move 2 from 8 to 9
move 4 from 3 to 5
move 2 from 1 to 7
move 1 from 3 to 6
...
const procedure = testString.split("\n")
    .map(line => line.split(/\D/).map(Number).filter(num => num > 0));
...
const rearrange = (arr, plan) => {
    ...
    for (let i = 0; i < plan.length; i++) {
        for (let j = arr[plan[i][1] -1].length; j > arr[plan[i][1] -1].length - plan[i][0]; j--) {
                    arr[plan[i][2] - 1].push(arr[[plan[i][1] -1]][j-1]);
        }
